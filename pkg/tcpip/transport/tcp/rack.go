...
func (s *sender) shouldSchedulePTO() bool {
	var opt tcpip.TCPRecovery
	if err := s.ep.stack.TransportProtocolOption(ProtocolNumber, &opt); err != nil {
		panic(fmt.Sprintf("can not determine if RACK recovery is enabled: %v", err))
	}
	return opt == tcpip.TCPRACKLossDetection && // Gate scheduling PTO using this option.
